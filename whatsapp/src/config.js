...
export function loadPersona(chatName) {
  if (!chatName) return null;
  const sanitized = chatName.replace(/[^a-zA-Z0-9_-]/g, "_");
  const filePath = join(getPersonasDir(), `${sanitized}.md`);
  try {
...
/**
 * Load persona content by group JID (looks up DB, reads file, extracts body after frontmatter).
 */
...
/**
 * Save/update a persona file and DB mapping.
 */
...
/**
 * Delete a persona file and DB mapping.
 */
